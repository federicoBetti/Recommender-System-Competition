...
cdef class SLIM_BPR_Cython_Epoch:
    ...
    def __init__(self, URM_mask, old_similarity=None,
                 train_with_sparse_weights = False,
                 final_model_sparse_weights = True,
                 learning_rate = 0.001, li_reg = 0.01, lj_reg = 0.01,
        ...
        if self.train_with_sparse_weights:
            ...
            if old_similarity is not None:
                for ind, i in enumerate(old_similarity):
                    columns = i.indices
                    data = i.data
                    for j in range(len(columns)):
                        self.S_symmetric.add_value(ind, columns[j], data[j])
                print("Similarity Matrix initialized with old matrix!")
        else:
