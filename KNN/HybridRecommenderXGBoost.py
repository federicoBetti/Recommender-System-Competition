...
def get_test_items(user_id, URM_validation):
    return URM_validation.indices[URM_validation.indptr[user_id]:URM_validation.indptr[user_id + 1]]


class HybridRecommenderXGBoost_Fede(SimilarityMatrixRecommender, Recommender):
    """ Hybrid recommender"""

    RECOMMENDER_NAME = "HybridRecommenderXGBoost_Fede"
    ...
    def __init__(self, URM_train, ICM, recommender_list, URM_PageRank_train=None,
        ...
        # parameters for xgboost
        self.label_XGboost = None
        self.xgbModel = XGBoost_model
        self.xgb_model_ready = XGB_model_ready
        self.tracks = tracks
        self.UCM_dense = self.UCM_train.todense()
        self.ICM_dense = self.ICM.todense()
        self.dict_song_pop = ged.tracks_popularity(self.URM_train)
    ...
    def recommend(self, user_id_array, dict_pop=None, cutoff=None, remove_seen_flag=True, remove_top_pop_flag=False,
            ...
            if self.label_XGboost is None:
                ...
                for song in predicted_songs:
                    if song in test_items:
                        ...
                        user_label.append(0)
                self.label_XGboost = np.concatenate([self.label_XGboost, user_label], axis=0)
        ...
        song_pop = np.array([[self.dict_song_pop[item] for item in relevant_line]
                             for relevant_line in relevant_items_boost.tolist()]).reshape((-1, 1))

        playlist_length = np.array([[int(ged.lenght_playlist(get_test_items(user, self.URM_train)))] * cutoff_Boost
                                    for user in user_list]).reshape((-1, 1))
        playlist_pop = np.array(
            [[int(ged.playlist_popularity(get_test_items(user, self.URM_train), self.dict_song_pop))] * cutoff_Boost
             for user in user_list]).reshape((-1, 1))
        ...
        if self.xgb_model_ready:
            ...
            current_user_id = 0
            for track_idx in range(newTrainXGBoost.shape[0]):
                ordered_tracks.append((relevant_items_boost[track_idx], preds[track_idx]))

                if track_idx % cutoff_Boost == 0 and track_idx != 0:
                    ordered_tracks.sort(key=lambda elem: elem[1], reverse=True)
                    ordered_tracks = [track_id[0] for track_id in ordered_tracks][:10]
